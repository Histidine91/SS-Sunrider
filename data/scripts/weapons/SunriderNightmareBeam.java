...
public class SunriderNightmareBeam implements BeamEffectPlugin {
			...
			if (fireInterval.intervalElapsed()) {
				...
				if (!hitShield || piercedShield) {
					...
					if (Math.random() > 0.5f) {
						...
						float dam = beam.getWeapon().getDamage().getDamage() * 0.01f;
						float empdam = beam.getWeapon().getDamage().getFluxComponent() * 0.02f;
